...
`persona.maskText`는 사용자가 지정한 이름들 대신 표시할 기본 문자열입니다. 사용자는 메모 화면에서 본명·애칭·호칭 등 원본 이름을 최대 20개까지 추가하고, 다른 대체 문구를 직접 입력할 수도 있습니다. 모든 원본 이름은 같은 대체 문구와 `persona.style`을 사용합니다.
