...
## v66.2
- 로그 이름 치환 세트의 원본 이름 중복 저장을 차단하고, 다섯 세트 버튼을 간결한 타일로 정리
- 퀵메뉴 내장 아이콘 그림 크기를 이전 값의 80%로 조정

## v66.1
- 사용자 지정 테마에 작은 섹션 제목 배경·메모 제목 색상 역할과 JSON 저장/불러오기를 추가
- 저장한 자유 색상은 길게 눌러 삭제 가능
- 로그 이름 바꾸기 세트의 원본 이름 중복을 실시간 안내 및 저장 전 확인
...
루미잉크는 프로젝트 단위로 여러 종류의 메모를 관리하는 로컬 웹앱/PWA입니다. 이 배포본의 사용자 버전은 **v1.5**이며, 기능 기준 개발 빌드는 **v66.1**입니다.

## 현재 버전

- 사용자 배포 버전: **v1.5**
- 개발 기준 빌드: `v66.3`
- 서비스워커 캐시: `ink-memo-v1.5-v66.3-quickmenu-theme-logstyle`
- 상세 매뉴얼: `Lumi_Ink_Manual_1.html`
- 최신 사용자 릴리스 노트: `Lumi_Ink_v1.5_Release_Notes.md`
- 개발 변경 이력: `Lumi_Ink_v66.3_Release_Notes.md`
