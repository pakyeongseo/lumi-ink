...
<html lang="ko"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><meta name="generator" content="Lumink"><meta name="lumink-kind" content="character"><title>${esc(n.title || (characterMode(n) === "single" ? "페르소나" : "캐릭터 모음"))}</title><style>${css}</style></head><body><main class="wrap"><h1 class="ptitle">${esc(n.title || (characterMode(n) === "single" ? "페르소나" : "캐릭터 모음"))}</h1>${nav}${pageHtml}<div class="foot">Lumi Ink · ${characterMode(n) === "single" ? "페르소나 카드" : "캐릭터 모음"}</div></main><script type="application/json" id="lumink-character">${payload}<\/script><script>(function(){var pages=[].slice.call(document.querySelectorAll('.char-page')),buttons=[].slice.call(document.querySelectorAll('.cnav button'));function show(i){pages.forEach(function(p,x){p.hidden=x!==i});buttons.forEach(function(b,x){b.classList.toggle('active',x===i)});window.scrollTo({top:0,behavior:'smooth'})}buttons.forEach(function(b){b.addEventListener('click',function(){show(+b.dataset.page)})})})();<\/script></body></html>`;
  ...
  const IDEA_KIND_LABEL = { note:"메모지", image:"이미지", audio:"음악", video:"영상", file:"파일", quote:"링크" };
  ...
  function ideaBoardSummary(n) {
    ...
    const labels = { note: "메모지", image: "이미지", audio: "음악", video: "영상", file: "파일", quote: "링크" };
  ...
  function setIdeaItemGeometry(el, item) {
    ...
    const baseH = item.kind === "audio" ? 82 : (["file","quote"].includes(item.kind) ? 62 : (item.kind === "video" ? 260 : (item.kind === "image" ? 255 : 190)));
  ...
  function bindIdeaNoteRichEditor(el,item,editor) {
    ...
    tools.addEventListener("pointerdown",(e)=>{e.stopPropagation();savedRange=captureIdeaNoteRange(editor);const button=e.target.closest("button");if(button&&button.dataset.ideaNoteCmd!=="hiliteColor"){e.preventDefault();command(button.dataset.ideaNoteCmd||"",button.dataset.ideaNoteValue||"");}});
    tools.addEventListener("click",(e)=>{e.stopPropagation();const button=e.target.closest("button");if(button&&button.dataset.ideaNoteCmd!=="hiliteColor"&&e.detail===0)command(button.dataset.ideaNoteCmd||"",button.dataset.ideaNoteValue||"");});
  ...
  function refreshIdeaMediaFrame(el, item) {
    ...
    const markup = ideaMediaFrameMarkup(item);
    if (frame) frame.innerHTML = markup;
  ...
  function openIdeaArtifactPreview(id) {
    ...
    let body = "";
    if (item.kind === "note") body = `<div class="idea-preview-note idea-sticky" data-color="${esc(item.color)}" data-note-style="${esc(item.noteStyle)}" style="${ideaColorStyleAttr(item.color, item.textColor)}"><div class="idea-note-text">${ideaNoteHtml(item) || "내용 없는 메모지"}</div></div>`;
  ...
  function openIdeaItemOptions(id) {
    ...
    const noteOptions=item.kind==="note"?`<div class="idea-options-section"><div class="idea-options-label">메모지 디자인</div><button class="idea-options-row" id="ideaOptDesign"><span>✦</span><span><b>${esc(IDEA_NOTE_TEMPLATES[item.noteStyle].label)}</b><small>디자인 선택 후 색상·글자색을 고르기</small></span></button></div>`:"";
    ...
    if(["quote","file","audio"].includes(item.kind))$on("ideaOptRename","click",()=>renameIdeaItemTitle(id));
    if(item.kind==="audio")$on("ideaOptAudioTitle","click",()=>{pushIdeaUndo();item.showTitle=item.showTitle===false;renderIdeaBoard();scheduleIdeaSave(0);closeModal();requestAnimationFrame(()=>openIdeaItemOptions(id));});
    if(["image","video"].includes(item.kind))$on("ideaOptFrame","click",()=>openIdeaMediaFramePicker(id));
  ...
  function renameIdeaItemTitle(id) {
    const n=currentIdeaNote(), item=getIdeaItem(id); if(!n||!item)return;
    const fallback=item.kind==="quote"?(getNote(item.noteId)?.title||"연결된 메모"):(item.kind==="audio"?((itemAttachment(n,item.fileId)?.name)||"오디오 트랙"):(itemAttachment(n,item.fileId)?.name||"첨부 파일"));
  ...
  function renderIdeaExportItem(item,n,fileMap) {
    ...
    if(item.kind==="video") return `<article class="${cls}${frameConfig?" has-media-frame":""}" style="${esc(base+frameStyle)}"><div class="idea-media-shell${frameConfig?" has-project-frame":""}"><div class="idea-media-content">${source?`<video controls playsinline src="${esc(source)}"></video>`:`<span>동영상 파일 없음</span>`}</div>${frameMarkup}</div></article>`;
